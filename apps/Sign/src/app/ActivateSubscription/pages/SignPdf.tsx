...
import {
  BackgroundsLayers,
  ElementsSign,
  ElementsSignChangeInfo,
  ElementsSignChangeInfo2,
  ElementsSignChangeSim,
  Layers,
} from './styles';
...
import { useDetailContractChangeInfo } from '../hooks/useDetailContractChangeInfo';
import { useCheckSkySaleContact } from '../hooks/useCheckSkySaleContact ';
...
const SignPdf: React.FC<any> = () => {
  ...
  const timeStampContract = new URLSearchParams(location.search)?.get(
    'timeStampContract'
  );
  ...
  const { data: dataDetailChangeInfo } = useDetailContractChangeInfo({
    id,
  });
  const { mutate: mutateSubmit, isPending: isLoadingSubmit } =
    useSubmitContract();
  const { data: dataCheckSkySaleContract } = useCheckSkySaleContact(id ?? '');
  const [checkSkySale, setCheckSkySale] = useState(false);
  useEffect(() => {
    if (
      dataCheckSkySaleContract &&
      dataCheckSkySaleContract.idNo &&
      dataCheckSkySaleContract.sessionId
    ) {
      form.setFieldsValue({
        idNo: dataCheckSkySaleContract?.idNo,
        sessionId: dataCheckSkySaleContract?.sessionId,
      });
      setCheckSkySale(true);
    }
  }, [dataCheckSkySaleContract, form, setCheckSkySale]);
  const handleFinish = () => {
  ...
  const elementsSign = () => {
    if (location.pathname === pathRouterSign.changeInfo)
      return (
        <ElementsSignChangeInfo>
    ...
    if (location.pathname === pathRouterSign.changeSim)
      return (
        <ElementsSignChangeSim>
          {signPng.length > 0 && <img src={signPng} alt="Signature" />}
        </ElementsSignChangeSim>
  ...
  const handleDownloadFileVBCT = () => {
    const isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent);
    if (isIOS) {
      // Tạo một thẻ a ẩn để tải xuống
      const link = document.createElement('a');
      link.href = '/file/template/noi_dung_chi_tiet_nd13.pdf';
      link.download = 'chinh_sach_VBCT.pdf';
  ...
  const handleDownloadFileNĐ13 = () => {
    const isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent);
    if (isIOS) {
      // Tạo một thẻ a ẩn để tải xuống
      const link = document.createElement('a');
      link.href = `/contract-decree/view-before/${id}`;
      link.download = 'Bien_ban_xac_nhan_ND13.pdf';
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
    } else {
    ...
    dataDetail && (
      <Form form={form} layout="vertical" onFinish={handleSubmit}>
        <div>
          ...
          {location.pathname === pathRouterSign.changeInfo &&
            dataDetailChangeInfo && (
              <Layers>
                <BackgroundsLayers>
                  <img
                    src={URL.createObjectURL(dataDetailChangeInfo)}
                    alt="Card"
                  />
                </BackgroundsLayers>
                <ElementsSignChangeInfo2>
        ...
        <div className="sticky z-10 bg-gray-200 bottom-0">
          <div className="flex flex-col justify-center mx-auto w-[400px] gap-2 py-4 relative">
            {location.pathname !== pathRouterSign.changeSim && (
              <>
                <div className="w-fit" onClick={handleDownloadFileVBCT}>
                  <Typography.Link underline target="_blank">
                    Biểu mẫu chính sách/VBCT Nghị định 13
                  </Typography.Link>
                </div>
                <div className="w-fit" onClick={handleDownloadFileNĐ13}>
                  <Typography.Link underline target="_blank">
        ...
        <Form.Item name="idNo" hidden label="idNo" />
        <Form.Item name="sessionId" hidden label="sessionId" />
      </Form>
    )
  );
};
