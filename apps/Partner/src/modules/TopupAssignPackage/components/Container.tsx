...
import { getColumnsTableTopupAssignPackage } from '../constants';
import useList, { TOPUP_ASSIGN_PACKAGE_QUERY_KEY } from '../hook/useList';
...
const Container = () => {
  ...
  const { PROCESS_STATUS_STATUS = [] } = useGetDataFromQueryKey<ParamsOption>([
    REACT_QUERY_KEYS.GET_PARAMS,
  ]);
  const columns = useMemo(() => {
    return getColumnsTableTopupAssignPackage(
      params,
      handleDownloadFile,
      PROCESS_STATUS_STATUS
    );
  }, [params, handleDownloadFile, PROCESS_STATUS_STATUS]);
  const { data: dataReport, isLoading } = useList(params);
  ...
  const { handleSearch } = useSearchHandler(TOPUP_ASSIGN_PACKAGE_QUERY_KEY);
  useEffect(() => {
    if (params) {
  ...
  return (
    <>
      <TitleHeader>Nạp tiền và gán gói trừ TKC</TitleHeader>
      <RowHeader>
        <Form form={form} onFinish={handleSubmit}>
          <Row gutter={[8, 8]}>
            <Col>
              <CFilter
                validQuery={TOPUP_ASSIGN_PACKAGE_QUERY_KEY}
                searchComponent={
        ...
        <CButtonAdd
          onClick={() => navigate(pathRoutes.topupAssignPackageAdd)}
        />
      </RowHeader>
      <CTable
        loading={isLoading}
