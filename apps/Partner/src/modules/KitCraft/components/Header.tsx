...
import { KitProcessTypeEnum } from '../types';
...
const Header: React.FC = () => {
  ...
  const { processType, time } = Form.useWatch((e) => e, form) ?? {};
  ...
  const items: ItemFilter[] = [
    ...
    },
    {
      label: 'Loại ghép KIT',
      value: (
        <Form.Item label="" name="processType" className="w-48">
          <CSelect
            options={
              allParams?.COMBINE_KIT_PROCESS_TYPE?.filter(
                (e) => e.value !== KitProcessTypeEnum.ONLINE
              ) ?? []
            }
          ...
          <Row gutter={8}>
            <Col>
              <CFilter
                ...
                searchComponent={
                  <CTooltip title="Nhập số thuê bao" placement="right">
