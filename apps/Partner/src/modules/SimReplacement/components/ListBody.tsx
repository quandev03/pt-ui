...
const ListBody = () => {
  ...
  const getDownloadUrl = (id: number, fileType: string) => {
    return `${prefixCustomerServicePublic}/change-sim-bulk/download-file?fileType=${fileType}&id=${id}`;
  };
  ...
  const columns: ColumnsType<ISimReplacementItem> = [
    ...
    },
    {
      ...
      render(_, record) {
        ...
        return record.processStatus === ProcessStatusEnum.COMPLETED ? (
          <Tooltip title={renderedValue} placement="topLeft">
            <Text>{`Số lượng thành công: ${
              record.succeededNumber ?? ''
            }`}</Text>
          ...
          <WrapperActionTable>
            ...
            <div className="w-5">
              {(includes(actionByRole, ActionsTypeEnum.CANCEL) ||
                ...
                renderMenuItemsMore(record)!.length > 0 && (
                  <Dropdown
