...
const fetcher = (params: IVerificationParams) => {
  ...
  const requestParams = {
    ...params,
    size: 10000,
    ...(processedSearchText && { searchText: processedSearchText })
  };

  return axiosClient.get<IVerificationParams, IPage<IVerificationItem>>(
    `${prefixCustomerService}/get-sub-document-admin`,
    { params: requestParams }
  );
};
export const useList = (params: IVerificationParams) => {
  const { isAdmin } = useCensorshipStore();
  return useQuery({
    queryKey: [REACT_QUERY_KEYS.GET_CENSORSHIP_LIST, params],
    queryFn: () => fetcher(params),
    enabled: !!params.type && isAdmin,
  });
};
