...
const ModalAdd = () => {
  ...
  const { mutate: addSimReplacement } = useAdd(form);
  ...
  useEffect(() => {
    if (supplier && orgPartnerOptions) {
      const partner = orgPartnerOptions.find((org) => org.orgCode === supplier);
      form.setFieldValue('partnerName', partner?.orgName);
      form.setFieldValue('email', partner?.email);
    }
  }, [supplier, orgPartnerOptions]);
  const handleClose = () => {
    navigate(-1);
  };
  ...
  const handleDownloadTemplate = () => {
    downloadTemplate({
      uri: `${prefixCustomerService}/change-sim-bulk/download-file?fileType=TEMPLATE`,
    ...
    <>
            ...
            <Col span={12}>
              ...
              >
                <CSelect
                  ...
                  placeholder="Chọn NPP yêu cầu"
                />
              </Form.Item>
              <Form.Item name={'partnerName'} hidden></Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item
                label="Email"
                required={form.getFieldValue('simType') === '2'}
                rules={[
                  {
                    validator(_, value) {
                      if (!value && form.getFieldValue('simType') === '2') {
                ...
                name={'email'}
              >
                <CInput maxLength={100} placeholder="Nhập thông tin email" />
              </Form.Item>
            </Col>
            <Col span={24}>
              <Form.Item
      ...
      <RowButton className="my-6">
        {includes(actionByRole, ActionsTypeEnum.CREATE) && (
          <CButton htmlType="submit" onClick={() => form.submit()}>
            Thực hiện
          </CButton>
        )}
