...
const fetcher = (body: any) => {
    return axiosClient.post<any, any>(
        `sale-service/public/api/v1/delivery-order/search-receive?page=0&size=20`,
        body
    );
};

export const useGetFilterDeliveryNoteSearch = (onSuccess: (data: any) => void) => {
    return useMutation({
        mutationFn: async (keySearch: string) => {
            const res = await fetcher({
                ...
                deliveryOrderMethod: DeliveryOrderMethod.EXPORT,
                deliveryNoteMethod: DeliveryOrderMethod.EXPORT
            });
