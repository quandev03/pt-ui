...
export const getColumnsTableInternalExportProposal = (
  params: IParamsRequest,
  {
    ...
    getLabelStatusApproval: (value: number) => string | undefined;
    getLabelStatusOrder: (value: number) => string | undefined;
  },
  {
    onCopy,
    onCancel,
  ...
  const renderMenuItemsMore = (record: AnyElement): AnyElement => {
    const arr = [
      {
        key: ActionsTypeEnum.UPDATE,
        label: (
          <Text className="w-full" onClick={() => onCopy(record)}>
            Sao chép
          </Text>
        ),
      },
    ...
    if (
      record.orderStatus === DeliveryOrderStatusList.CREATE &&
  ...
  return [
    ...
    },
    {
      ...
      render(value: number) {
        return (
          <Tooltip placement="topLeft" title={getLabelStatusOrder(value)}>
            <CTag color={getColorStatusOrder(value)}>
              {getLabelStatusOrder(value)}
            </CTag>
          </Tooltip>
        );
      },
