...
const fetcher = async (data: any) => {
  ...
  const deliveryNote = {
    ...
    ticketOutDeliveryNoteId: data.ticketOutDeliveryNoteId,
    attachments: data.attachmentsDTOS,
  ...
  const res = await axiosClient.post(`${urlInternalWarehouseDeliveryNote}/create-ticket-in`, formData, {
    headers: {
