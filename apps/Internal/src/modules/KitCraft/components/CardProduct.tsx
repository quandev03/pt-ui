...
const CardProduct: React.FC<ProductProps> = ({ actionType, idx }) => {
  ...
  const getProfileTypeOpts = useMemo(() => {
    return products[idx]?.profileTypeList?.map((value: string) => {
  ...
  }, [products[idx]?.profileTypeList, profileTypeOpts]);
  return (
        ...
        <CCol xxl={{ flex: 20 }} span={8}>
          ...
          >
            <DebounceSelect
              ...
              //@ts-ignore
              fetchOptions={!isOrderd ? mutateProductList : undefined}
              disabled={isViewType || isOrderd}
