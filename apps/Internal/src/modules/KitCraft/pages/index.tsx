...
const KitCraftPage: React.FC = () => {
  ...
  const columns: TableColumnsType<KitCraftType> = [
    ...
    },
    {
      ...
      width: 130,
      render: (value: string) => {
        const processType = allParams?.COMBINE_KIT_PROCESS_TYPE?.find(
          (e) => e.value === String(value)
        )?.label;
        return <CTooltip title={processType}>{processType}</CTooltip>;
      },
    },
    {
      title: 'Mã đơn hàng',
    ...
    },
    {
      ...
      render: (value: string, record: KitCraftType) => {
        if (
          record.processType !== KitProcessType.BATCH ||
