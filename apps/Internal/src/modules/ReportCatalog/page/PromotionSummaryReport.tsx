...
import {
  IParamsPromotionSummaryReport,
  useGetContentPromotionSummaryReport,
} from '../hook/useGetContentPromotionSummaryReport';

const PromotionSummaryReport = () => {
  const [searchParams, setSearchParams] = useSearchParams();
  const params = decodeSearchParams(searchParams);
  const actionByRole = useRolesByRouter();
  const [form] = Form.useForm();

  const { data: content, isLoading } = useGetContentPromotionSummaryReport({
    ...queryParams({
      ...params,
    }),
  });
  ...
  const handleDownload = () => {
    downloadReport({
      ...
      url: `${prefixReportService}/promotion-program/summary/export`,
    });
  };
  ...
  const items: ItemFilter[] = [
    {
      ...
      showDefault: true,
      value: (
        <Form.Item className="min-w-52" name="promMethod">
          <CSelect
  ...
  return (
    <Wrapper>
      <TitleHeader>Báo cáo tổng hợp mã khuyến mại</TitleHeader>
      <RowHeader>
        <Form onFinish={handleSubmit} form={form}>
          <CFilter
            ...
            searchComponent={
              <Tooltip title="Nhập tên mã KM để tìm kiếm" placement="topLeft">
                <Form.Item name="valueSearch" className="min-w-72">
                  <CInput
                    maxLength={50}
                    placeholder="Nhập tên mã KM để tìm kiếm"
...
export default PromotionSummaryReport;
