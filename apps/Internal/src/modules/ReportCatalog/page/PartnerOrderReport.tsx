...
import {
  IParamsPartnerOrderReport,
  useGetContentPartnerOrderReport,
} from '../hook/useGetContentPartnerOrderReport';

const PartnerOrderReport = () => {
  const [searchParams, setSearchParams] = useSearchParams();
  const params = decodeSearchParams(searchParams);
  const actionByRole = useRolesByRouter();
  const [form] = Form.useForm();
  const { data: content, isLoading } = useGetContentPartnerOrderReport({
    ...queryParams({
      ...params,
  ...
  const { SALE_ORDER_STATUS = [] } = useGetDataFromQueryKey<ParamsOption>([
  ...
  return (
    <Wrapper>
      <TitleHeader>Báo cáo chi tiết đơn hàng đối tác</TitleHeader>
      <RowHeader>
        <Form
        ...
        >
          <CFilter
            ...
            searchComponent={
              <Tooltip
                title="Nhập mã đơn hàng đối tác, tên đối tác để tìm kiếm"
                placement="topLeft"
              >
                <Form.Item name="q" className="min-w-72">
                  <CInput
                    maxLength={50}
                    placeholder="Nhập mã đơn hàng đối tác, tên đối tác để tìm kiếm"
...
export default PartnerOrderReport;
