...
import {
  IParamsInventoryExportReport,
  useGetContentInventoryExportReport,
} from '../hook/useGetContentInventoryExportReport';
...
const InventoryExportReport = () => {
  ...
  const { data: content, isLoading } = useGetContentInventoryExportReport({
    ...queryParams({
      ...params,
  ...
  const { data: optionPartner = [], isLoading: loadingPartnerOption } =
    useGetAllPartner();

  const optionsTransactionType = useMemo(() => {
    return STOCK_MOVE_LOOK_UP_MOVE_TYPE.filter(
      (item) =>
        item.value === TypeTransaction.ExportInternal ||
        item.value === TypeTransaction.ExportOther ||
        item.value === TypeTransaction.ExportPartner ||
        item.value === TypeTransaction.ExportSIM ||
        item.value === TypeTransaction.ExportKIT
    );
  ...
  const handleDownload = () => {
    downloadReport({
      ...
      url: `${prefixReportService}/stock-out/export`,
    });
  };
  ...
  const items: ItemFilter[] = [
    ...
    },
    {
      label: 'Chọn đối tác',
      value: (
        <Form.Item className="min-w-52" name="clientId">
          <CSelect
            loading={loadingPartnerOption}
  ...
  return (
    <Wrapper>
      <TitleHeader>Báo cáo xuất kho</TitleHeader>
      <RowHeader>
        <Form
        ...
        >
          <CFilter
            ...
            searchComponent={
              <Tooltip title="Nhập số xuất kho" placement="topLeft">
                <Form.Item name="stockCode" className="min-w-72">
                  <CInput
                    maxLength={50}
                    placeholder="Nhập số xuất kho"
